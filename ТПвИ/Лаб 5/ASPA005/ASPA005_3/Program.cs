...
internal class Program
{
	private static void Main(string[] args)
	{
		...
		app.MapPost("/A/{x:min(0):max(100)}", (HttpContext context, [FromRoute] int x) => Results.Ok(new { path = context.Request.Path.Value, x = x }));
		...
		app.MapDelete("/A/{x:min(1)}-{y:min(1):max(100)}", (HttpContext context, [FromRoute] int x, [FromRoute] int y) => Results.Ok(new { path = context.Request.Path.Value, x = x, y = y }));
		...
		// --- F ---
		//--- F -----------
		app.MapGet("/F/{x:regex(^\\S+@\\S+\\.by$)}", (HttpContext context, [FromRoute] string x) => Results.Ok(new { path = context.Request.Path.Value, x = x }));
