...
namespace WPF_FitnessClub.DataBase
{
    public static class MigrationManager
    {
        public static void ApplyMigrations(AppDbContext context)
        {
            try
            {
                ...
                // Если есть хотя бы одна таблица, но не все - возможно БД повреждена
                ...
                // Применяем миграции для WorkoutPlans
                ...
                // Применяем миграции для NutritionPlans
                ...
                // Применяем миграции для TrainingPlans
        ...
        private static void ApplyTableMigration(AppDbContext context, string tableName, string createTableSQL, string requiredColumn)
        {
            try
            {
                ...
                {
                    // Если таблица существует, но обязательная колонка отсутствует, добавляем ее
            ...
            catch (Exception ex)
            {
                ...
                if (ex.Message.Contains("There is already an object named"))
                {
                    ...
                    return; // Игнорируем ошибку, если таблица уже существует
                }
    ...
    // Класс конфигурации миграций (оставляем для совместимости)
    public sealed class Configuration : DbMigrationsConfiguration<AppDbContext>
    {
        public Configuration()
        {
            AutomaticMigrationsEnabled = true;
            AutomaticMigrationDataLossAllowed = true; // Разрешаем потенциальную потерю данных при миграции
        }
