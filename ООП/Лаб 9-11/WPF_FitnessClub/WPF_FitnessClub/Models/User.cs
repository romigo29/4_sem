	...
	public class User : BaseEntity
	{
		...
		public UserRole Role
		{
			...
			set
			{
				if (role != value)
				{
					// Проверяем, что значение является допустимым для enum UserRole
					if (!Enum.IsDefined(typeof(UserRole), value))
					{
						role = UserRole.Client; // По умолчанию устанавливаем Client
					}
					else
					{
						role = value;
		...
		// Свойство для строкового представления роли (для отображения в интерфейсе)
		[NotMapped]
