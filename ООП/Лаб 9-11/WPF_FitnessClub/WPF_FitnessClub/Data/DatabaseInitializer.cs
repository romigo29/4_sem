    ...
    public class DatabaseInitializer
    {
        ...
        public bool InitializeDatabase()
        {
            try
            {
                ...
                if (isConnected)
                {
                    ...
                    using (var context = new AppDbContext())
                    {
                        try
                        {
                            ...
                            // Если база существует, проверяем ее структуру и применяем миграции
                            try
                            {
                                // Сначала пытаемся применить миграции к существующей базе
                                MigrationManager.ApplyMigrations(context);
                                
                                // Если миграции успешно применены, делаем бэкап данных
                            ...
                            {
                                // Если не удалось применить миграции, логируем ошибку
                                ...
                                // Логируем ошибку в файл для последующего анализа
                                ...
                                // Возвращаем false, что означает необходимость пересоздания базы
                                return false;
                            }
                        }
                ...
                {
                    // Если подключение неуспешно или базы не существует
            ...
            try
            {
                ...
                using (var context = new AppDbContext())
                {
                    try
                    {
                        ...
                        // Применяем миграции для добавления необходимых таблиц
                        MigrationManager.ApplyMigrations(context);
                    ...
                    // Сначала пробуем восстановить данные из JSON файла
                    bool restoredFromJson = RestoreFromJsonFile(context);
                    
                    // Если не удалось восстановить из JSON, используем другие методы
                    if (!restoredFromJson)
                    {
                        // Загружаем данные, если они были сохранены
                        if (_hasExistingData)
                        {
                            RestoreData(context);
                        }
        ...
        private bool RestoreFromJsonFile(AppDbContext context)
        {
            try
            {
                ...
                // Для упрощения работы с ID создаем словари соответствия
                ...
                // Восстанавливаем UserSubscriptions
                ...
                // Тут можно добавить восстановление остальных таблиц по аналогии...
        ...
        private void RestoreData(AppDbContext context)
        {
            try
            {
                ...
                if (_cachedUsers.Count > 0)
                {
                    ...
                    {
                        // Сбрасываем ID, чтобы EF не пытался обновить существующую запись
                        var newUser = new User
                        {
                            Login = user.Login,
                            Password = user.Password,
                ...
                // Восстанавливаем другие данные по аналогичному принципу...
                // (для краткости кода здесь не реализованы все остальные таблицы)
