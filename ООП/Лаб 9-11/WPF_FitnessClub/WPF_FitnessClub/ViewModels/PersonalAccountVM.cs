	...
	public class PersonalAccountVM : ViewModelBase
	{
		...
		public string Username
		{
			...
			set
			{
				...
				// Сбрасываем ошибку при изменении Username
				if (IsUsernameError)
				{
		...
		public string Email
		{
			...
			set
			{
				...
				// Сбрасываем ошибку при изменении Email
				if (IsEmailError)
				{
					IsEmailError = false;
		...
		public string FullName
		{
			...
			set
			{
				...
				// Сбрасываем ошибку при изменении FullName
				if (IsFullNameError)
				{
		...
		public string ConfirmPassword
		{
			...
			set
			{
				...
				// Сбрасываем ошибку при изменении подтверждения пароля
		...
		public void LoadWorkoutPlans()
		{
			try
			{
				...
				// Загружаем планы тренировок в зависимости от роли пользователя
		...
		public void LoadNutritionPlans()
		{
			try
			{
				...
				// Загружаем планы питания в зависимости от роли пользователя
				if (_user.Role == UserRole.Client)
				{
		...
		private void ExecuteCancelSubscription(object parameter)
		{
			...
			{
				// Проверяем, можно ли отменить абонемент (не истёк ли срок его действия)
		...
		private void ExecuteSave(object parameter)
		{
			try
			{
				...
				{
					// Ошибка и сообщение уже установлены в ValidateEmail
				...
				{
					// Создаем строку сообщения напрямую, без использования ресурсов
					...
					return; // Прерываем выполнение, не сохраняя данные
				}
		...
		{
			// Кнопка смены пароля всегда активна
			return true;
		}
			...
			try
			{
				...
				if (validationErrors.Count > 0)
				{
					...
					MessageBox.Show(
						...
						MessageBoxButton.OK,
						MessageBoxImage.Warning);
					return; // Прерываем выполнение, не меняя пароль
				}
		...
		private void ExecuteMarkWorkoutPlanCompleted(object parameter)
		{
			if (parameter is WorkoutPlan workoutPlan)
			{
				try
				{
					...
					// Получаем актуальную версию плана из базы данных
					...
					// Используем текущее значение из UI чекбокса напрямую
					...
					// Проверяем, что значение соответствует тому, что мы хотели установить
					if (updatedPlan.IsCompleted != newStatus)
					{
						...
						// Синхронизируем UI с фактическим значением из базы
